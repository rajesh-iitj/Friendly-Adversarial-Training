...
def train(model, train_loader, optimizer, tau):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        output_adv, output_target, output_natural, count = earlystop(model, data, target, step_size=args.step_size,
                                                                     ...
                                                                     tau=tau, randominit_type="normal_distribution_randominit", loss_fn='kl', rand_init=args.rand_init,
                                                                     omega=args.omega)
        bp_count += count
        model.train()
        optimizer.zero_grad()
...
if args.resume:
    ...
    print('==> Friendly Adversarial Training for TRADES')
